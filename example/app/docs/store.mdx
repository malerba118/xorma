...
In xorma, your state is held in a centralized store which you can think of as a reactive in-memory database.
...
So when we attempt to create the 2nd user, what actually happens under the hood is xorma sees the existing `UserModel` instance associated with id `user-1` and updates the instance's data by calling `instance.loadJSON({ id: "user-1", first_name: "Maxime", last_name: "Heckel" })`.
...
This means that as long as we wrap our component in the `observer` hoc, the component will automatically render whenever instances are added/removed.
