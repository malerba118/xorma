...
Next, you'll need to define some models.

Each model will operate on a specific object type, so for example the `UserModel` will operate on the `User` object type. You'll have to implement `loadJSON` and `toJSON` methods for each of your model types.
