...
class ProjectModel extends Model.withType(DataType<Project>) {
   ...
   * that the logic inside of this getter will rarely run 🙌.
   */
  get tasks(): TaskModel[] {
...
Though xorma works best with highly normalized data, you can also use it to work with embedded data.

In the following example our projects and tasks are modeled slightly differently where tasks are a subcollection on projects.
...
However, it is only recommended to use xorma with embedded data if you're working with pre-existing data.

If you're building a new application, it is highly recommended that you stick with relational data modeling (sql) over embedded data modeling (nosql).
