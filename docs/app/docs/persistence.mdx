...
This version will also be present in the serialized data returned by `store.toJSON`.

If the schemaVersion of the data ingested by store.loadJSON does nto match the schemaVersion specified when initializing the store, the store will throw an error.
...
Let's assume we have the following TaskModel. The status of the task is represented as a boolean `done` state.
...
But we're feeling this isn't experessive enough and instead of a boolean done field, we decide we want a status field which can be one of `["idle", "in-progress", "done"]`.
...
But now if we fetch a project with schemaVersion 0, obviously we can't load that into the store, because it has a done field and status will be undefined.
