...
To install xorma, you'll also need to install mobx.
...
Each model will operate on a specific object type, so for example the `UserModel` will operate on the `User` object type. To get started you'll have to implement `loadJSON` and `toJSON` methods for each of your model types.
