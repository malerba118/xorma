...
export class ImportFixer {
  ...
  public fix(document: Monaco.editor.ITextModel, imp: ImportObject): void {
    ...
    if (position.lineNumber <= 1) {
      ...
      this.editor.setPosition(
        new this.monaco.Position(
          position.lineNumber + offset,
          text.length + position.column
        )
      );
    } else {
      const offset = edits[0].range.endLineNumber > 0 ? 0 : 1;
      this.editor.executeEdits("", edits);
      this.editor.setPosition(
        new this.monaco.Position(position.lineNumber + offset, position.column)
      );
    }
  }
  ...
  private mergeImports(
    ...
    if (match) {
      ...
      const imports = [
        ...workingString.split(",").map((name) => name.trim()),
        imp.name,
      ];
      const newImport = this.createImportStatement(document, {
        name: imports.join(", "),
        path: path,
        type: imp.type,
      });
  ...
  private createImportStatement(
    document: Monaco.editor.ITextModel,
    imp: ImportObject | { name: string; path: string; type: Expression }
  ): string {
    const path =
    ...
    let currentDoc = document.getValue();
    const existsExp = new RegExp(
      `(?:import[ \t]+{)(?:.*)(?:}[ \t]+from[ \t]+['"])(?:${path})(?:['"])`
    );
    const doubleQuoteExp = new RegExp(
      `(?:import[ \t]+{)(?:.*)(?:}[ \t]+from[ \t]+")(?:${path})(?:")`
    );
    const spacesExp = new RegExp(
      `(?:import[ \t]+{[ \t]+)(?:.*)(?:[ \t]*}[ \t]+from[ \t]+['"])(?:${path})(?:['"])`
    );
    const semiColonExp = new RegExp(
      `(?:import[ \t]+{)(?:.*)(?:}[ \t]+from[ \t]+['"])(?:${path})(?:['"]);`
    );
